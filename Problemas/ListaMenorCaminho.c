    ...
    while(*aux){
        if ((*aux)->informacao.codAeroporto == p->informacao.codAeroporto){ //Se esse elemento ja estiver na lista...
            if (p->informacao.distancia <=  (*aux)->informacao.distancia){  //...e sua distancia for menor que a distancia atual
                (*aux)->informacao.distancia = p->informacao.distancia;     //...atualiza distância
                (*aux)->pai = pai;                                          //...e atualiza o nó pai
                return 1;
            }
...
lista_MN menorCaminho(grafo *g, TAD_ABB a, char *origem, char *destino){
    ...
    int i;
    //Criando lista
    lista_MN lista = criaLista_MN(2,origem,destino);  //Cria lista, defininda a inserção pela cauda
    if (!lista) { printf("    ERRO AO CRIAR LISTA DE MENOR CAMINHO!"); return NULL; }

    //Inserindo a cidade de origem
    dado inf;
    RegistroArvore *reg = TAD_ABB_busca(a,origem); //Busca informações da cidade de origem
    if (!reg){ printf("    ERRO AO BUSCAR CIDADE DE ORIGEM NA ÁRVORE!"); return NULL; }
    ...
    while (strcmp(no->informacao.cidade,destino)){  //Enquanto o menor caminho não for o que leva à cidade de destino...
        ...
        for (i=0;i<qtdaElementos(listaGrafo);i++){ //Para cada elemento da lista (que são as cidades conectada à cidade de menor distancia no mom...
    ...
    lista_MN listaResult = (criaLista_MN(1,origem,destino)); //Inserção pela cabeça
    while (no){
